...
export function Sidebar({ title, className, children, footer }: SidebarProps) {
  return (
    <aside className={"w-full shrink-0 sm:max-w-sm " + (className ?? "")}>
      <div className="flex flex-col gap-4 sm:[@media(min-height:640px)]:sticky sm:[@media(min-height:640px)]:top-20 sm:[@media(min-height:640px)]:max-h-[calc(100vh-6rem)]">
        <div className="flex min-h-0 flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-lg">
          ...
          <div className={(title ? "mt-4 " : "") + "min-h-0 sm:[@media(min-height:640px)]:overflow-y-auto pr-1"}>{children}</div>
        </div>
